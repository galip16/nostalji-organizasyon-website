...
export default function Pricing() {
  ...
  const calc = useMemo(() => {
    ...
    if (products.serbet) {
      const price = (basePrices.serbet + (servedPeople * 15));
      breakdown["Osmanlı Şerbeti"] = price;
      subtotal += price;
    }

    if (products.salep) {
      const price = (basePrices.salep + servedPeople * 10) * standMultiplier;
      breakdown["Salep"] = price;
      subtotal += price;
    }

    if (products.palyaco) {
      const price = palyacoCount * basePrices.palyaco;
      breakdown["Palyaço"] = price;
      subtotal += price;
    }
    ...
    if (productCount >= 3 || standMultiplier === 2) discountRate += 0.10;
    else if (productCount >= 2) discountRate += 0.05;

    if (products.palyaco && palyacoCount >= 2) discountRate += 0.05;

    if (time !== "aksam") discountRate += 0.05;
      ...
      <div className="max-w-4xl mx-auto bg-white p-8 rounded-2xl shadow">
        ...
        <div className="bg-pink-50 p-6 rounded-xl">
          ...
          <div className="flex justify-between text-green-600">
            <span>İndirim</span>
            <span>-{calc.discount.toFixed(0)} TL</span>
          </div>
